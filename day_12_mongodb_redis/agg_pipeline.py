...
def get_db():
    try:
        ...
        print("[ERROR] MongoDB not running! Run: docker run -d -p 27017:27017 --name mongodb mongo")
        exit(1)
    ...
    pipeline = [
        {"$addFields": {
            "performance_tier": {
                "$switch": {
                    "branches": [
                        {"case": {"$gte": ["$metrics.test_f1", 0.93]}, "then": "[BEST] Excellent"},
                        {"case": {"$gte": ["$metrics.test_f1", 0.90]}, "then": "[OK] Good"},
                        {"case": {"$gte": ["$metrics.test_f1", 0.85]}, "then": "[WARN]  Average"},
                    ],
                    "default": "[ERROR] Needs Work"
                }
            },
            "efficiency_score": {
